 ...
 */
class TwilioRouter {
    static async nextMessage(ctx, config, currentBlock) {
    ...
    static async handlePlaybackBlock(botId, blockName, response, ctx, nextBlock, messages) {
        ...
        switch (blockName) {
            ...
            case (TwilioTypes_1.BlockId.record_playback): {
                ...
                const recordings = await ctx.firebaseApi.getPendingRecordingsWithRetries(botId, ctx.callSid, 1, 8, 100);
     ...
     */
    static async gatherNextMessage(ctx, config, currentBlock, gatherResult) {
